...
e2function void entity:playerNoclip(status)
	if not self.player:GetNWBool("E2PowerAccess") then error( "[E2p]: у тебя нет доступа к playerNoclip()!" ) return end
	if !IsValid(this) then return end
	if !isOwner(self, this) then return end
	if !this:IsPlayer() then return end
...
e2function void entity:playerNoclipToggle() -- Zimon4eR
	if not self.player:GetNWBool("E2PowerAccess") then error( "[E2p]: у тебя нет доступа к playerNoclipToggle()!" ) return end
	if !IsValid(this) then return end
	if !isOwner(self, this) then return end
	if !this:IsPlayer() then return end
